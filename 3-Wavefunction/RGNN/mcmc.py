...
def mh_update(
  ...
  if atoms is None:  # symmetric proposal, same stddev everywhere
    ...
    lp_2 =  f(
        params, x2, data.spins, data.atoms, data.charges, wrams, key
...
def mh_block_update(
  ...
  if atoms is None:  # symmetric prop, same stddev everywhere
    ...
    if pad > 0:
      x2 = x2[..., :-pad*ndim]
    # log prob of proposal
    lp_2 = 2.0 * f(params, x2, data.spins, data.atoms, data.charges, wrams1)
    ratio = lp_2 - lp_1
  else:  # asymmetric proposal, stddev propto harmonic mean of nuclear distances
...
def make_mcmc_step(batch_network,
  ...
  @jax.jit
  def mcmc_step(params, data, key, width, wrams=None):
    ...
    nsteps = steps * blocks
    logprob =  batch_network(
        params, pos, data.spins, data.atoms, data.charges, wrams, key
    )
    logprob = logprob * 2.0
    ...
    return new_data, pmove#, params_layers
