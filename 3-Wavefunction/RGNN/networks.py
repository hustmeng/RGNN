...
from RGNN import jastrows
from RGNN import reservoir_layer
...
def _split_spin_pairs(
  ...
  up_up, up_down, down_up, down_down = reservoir_layer.split_into_blocks(
      arr, nspins
  )
...
def _combine_spin_pairs(
  ...
  same_pair_partitions = reservoir_layer.array_partitions(nsame_pairs)
...
def construct_symmetric_features(
  ...
  spin_partitions = reservoir_layer.array_partitions(nspins)
...
def make_schnet_convolution(
  ...
  def apply(
    ...
    h_one_embedding = reservoir_layer.linear_layer(h_one, params['single'])
    h_two_embeddings = [
        reservoir_layer.linear_layer(h_two_channel, layer_param)
...
def make_schnet_electron_nuclear_convolution() -> ...:
  ...
  def init(
    ...
    return {
        'electron_ion_embedding': reservoir_layer.init_linear_layer(
            key1,
            in_dim=electron_nuclear_dim,
            out_dim=embedding_dim,
            include_bias=False,
        )['w'],
        'ion_embedding': reservoir_layer.init_linear_layer(
...
def make_fermi_net_layers(
  ...
  """
  init_fn, apply_fn  = stax.BatchNorm()
  ...
  def init(key: chex.PRNGKey) -> Tuple[int, ParamTree]:
    ...
    if options.nuclear_embedding_dim:
      ...
      params['nuclear'] = reservoir_layer.init_linear_layer(
          nuclear_key,
          in_dim=1,
    ...
    for i in range(len(options.hidden_dims)):
      ...
      layer_params['single'] = reservoir_layer.init_linear_layer(
          single_key,
          in_dim=dims_one_in,
      ...
      if i < len(options.hidden_dims) - 1 or options.use_last_layer:
        ...
        if options.electron_nuclear_aux_dims:
          layer_params['electron_ion'] = reservoir_layer.init_linear_layer(
              aux_key,
  ...
  def apply_layer(
    ...
    # Execute next layer.
    key, subkey = jax.random.split(key)
    h_one_next = reservoir_layer.reservoir_layer(h_one_in, subkey, **params['single'])    
    h_one_next = jnp.tanh(h_one_next)
    
    h_one = residual(h_one, h_one_next)  ###   1
    ...
    if 'double' in params:
      ...
      for prev, param in zip(h_two, params_double):
          key, subkey = jax.random.split(key)
          h_two_next = reservoir_layer.reservoir_layer(prev, subkey, **param)
      h_two_next = [jnp.tanh(h_two_next)]

      h_two = tuple(residual(prev, new) for prev, new in zip(h_two, h_two_next))
    if h_elec_ion is not None and 'electron_ion' in params:
      h_elec_ion = reservoir_layer.linear_layer(
  ...
  def apply(
    ...
    if options.nuclear_embedding_dim:
      nuclear_embedding = reservoir_layer.linear_layer(
...
def make_orbitals(
  ...
  """
  key = jax.random.PRNGKey(30777)
  equivariant_layers_init, equivariant_layers_apply = equivariant_layers
  key, subkey = jax.random.split(key)
  _, params_layers = equivariant_layers_init(subkey)
  
  
  cfg = base_config.default()
  with np.load(cfg.Conductance_path,allow_pickle=True) as data:
     params_layers = data["layers"].tolist()
  ...
  def init(key: chex.PRNGKey) -> ParamTree:
    ...
    dims_orbital_in, _ = equivariant_layers_init(subkey)
  ...
  def apply(
    ...
    if options.envelope.apply_type == envelopes.EnvelopeType.PRE_ORBITAL:
      envelope_factor = options.envelope.apply(
          ae=ae, r_ae=r_ae, r_ee=r_ee, **params['envelope']    ##  2
      )
      h_to_orbitals = envelope_factor * h_to_orbitals    ## 3
    # Note split creates arrays of size 0 for spin channels without electrons.
    h_to_orbitals = jnp.split(
        h_to_orbitals, reservoir_layer.array_partitions(nspins), axis=0
    )
    # Drop unoccupied spin channels
    h_to_orbitals = [h for h, spin in zip(h_to_orbitals, nspins) if spin > 0]
    active_spin_channels = [spin for spin in nspins if spin > 0]
    active_spin_partitions = reservoir_layer.array_partitions(
        active_spin_channels
    )
    # Create orbitals.
    orbitals = [
        reservoir_layer.linear_layer(h, **p)
...
def make_fermi_net(
  ...
  def init(key: chex.PRNGKey) -> ParamTree:
    ...
    params, params_layers = orbitals_init(subkey)
    return params, params_layers
  ...
  def apply(
    ...
    orbitals, h_to_orbitals = orbitals_apply(params, pos, spins, atoms, charges, wrams, key)
    return reservoir_layer.logdet_matmul(orbitals)
