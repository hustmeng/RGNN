...
'''
h_data = np.zeros((13,13))
nov = 0 
for key, data in hamiltonians_pred.items():
...
plt.imshow(h_data, cmap=cmap)

    # 设置标题和坐标轴标签
    #plt.title('Heatmap')
plt.xlabel('Orbital $α$')
plt.ylabel('Orbital $β$')
plt.gca().invert_yaxis()
    # 添加颜色条
plt.colorbar()
plt.savefig("pdf.pdf",dpi = 600, bbox_inches='tight', format='pdf')
plt.close()

ov_data = np.zeros((13,13))
nov = 0 
for key, data in overlaps.items():
    print(key,data.shape)
    ov_data = ov_data + data
    nov = nov + 1
ov_data = ov_data / nov

cmap = plt.cm.get_cmap('YlGnBu')
plt.imshow(ov_data, cmap=cmap)
...
plt.gca().invert_yaxis()
    # 添加颜色条
plt.colorbar()
plt.savefig("Overlaps.pdf",dpi = 600,bbox_inches='tight', format='pdf')
plt.close()
'''
H_R = {}
S_R = {}
...
for key in hamiltonians_pred.keys():
    ...
    if R not in H_R.keys():  #Unit vector in lattice coordinates
...
'''
ov_data = np.zeros((1300,1300))
nov = 0 
for key, data in S_R.items():
    print(key,data.shape,data[:10,:10])
    ov_data = ov_data + data
    nov = nov + 1
ov_data = ov_data / nov
ov_data = np.real(ov_data)#[:100,:100]
#print(ov_data)
...
plt.gca().invert_yaxis()
    # 添加颜色条
plt.colorbar()
plt.savefig("Overlaps_100.pdf",dpi = 600, bbox_inches='tight', format='pdf')
plt.close()

H_data = np.zeros((1300,1300))
nov = 0 
for key, data in H_R.items():
    print(key,data.shape)
    H_data = H_data + data
    nov = nov + 1
H_data =  H_data / nov
H_data = np.real(H_data)#[:100,:100]
...
plt.gca().invert_yaxis()
    # 添加颜色条
plt.colorbar()
plt.savefig("H_100.pdf",dpi = 600, bbox_inches='tight', format='pdf')
...
def plot(data, name):
    ...
    plt.savefig(name + ".pdf", dpi = 600, bbox_inches='tight', format='pdf')

    plt.close()
'''
#print(H_R.keys(),H_R.keys())
...
if calc_job == "band":
    ...
    for i in range(len(kpaths)):
        ...
        for kx, ky, kz in zip(kxs, kys, kzs):
            ...
            for R in H_R.keys():   #Unit vector in lattice coordinates
                H_k += H_R[R] * np.exp(1j*2*np.pi*np.dot([kx, ky, kz], R))
                S_k += S_R[R] * np.exp(1j*2*np.pi*np.dot([kx, ky, kz], R))
            print("fuliye",time() - begin_time)     
       #     print(H_k.shape, S_k.shape) 
    ...
    '''
    plot(np.real(H_k),"H-k")
    plot(np.real(S_k),"S-k")
    
    cmap = plt.cm.get_cmap('YlGnBu')
    plt.imshow(egvals, cmap=cmap, vmin=-35, vmax=100)
    ...
    plt.colorbar()
    plt.gca().set_aspect('auto')
    plt.savefig("egval.pdf", dpi = 600, bbox_inches='tight', format='pdf')
    '''
    #plot(np.real(egvals[:30,:]), "egvals")
    #print(egvals)
